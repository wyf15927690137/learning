...
def AutoUpdate(server_ip, server_port, module_name, order):
    ...
    # override the file to the main directory
    prompt_info11.set("unzipping...")
    prompt_info13.set("total progress: 99.9%")
    prompt_info21.set("")
    root.update()
    ...
    prompt_info11.set("update complete")
    prompt_info13.set("the total progress: 100.0%")
    root.update()
    return True

# analyze the two xml files
def analyze_update_info(local_xml, update_xml, module_name):
    ...
    for module_name in module_names:
        ...
        # decide whether need to update according to the version
...
# download the files need to be updated
def download_progress(file_info_dict):
    ...
    try:
        ...
        for each_file in file_info_dict.keys():
            if "regressiontest/ClientUpdateTools" in each_file or "regressiontest/.git" in each_file or "regressiontest/.idea" in each_file or "regressiontest/bin" in each_file or "regressiontest/lib" in each_file:
                file_not_update.append(each_file)
                
        for each_file in file_not_update:
            file_info_dict.pop(each_file)
        ...
        for each_file in file_info_dict.keys():
            file_count += 1
            download_url = "http://{0}:{1}/RegressionTestTools/{2}".format(server_ip, server_port, each_file)
            ...
            with closing(requests.get(download_url, stream=True)) as response:
                ...
                content_size = int(file_info_dict[each_file])  # int(response.headers['content-length'])   
                data_count = 0
                with open(local_path, "wb") as file:
                    for data in response.iter_content(chunk_size=chunk_size):
                        file.write(data)
                        data_count = data_count + len(data)
                        prompt_info11.set("download files: (%d/%d)" % (file_count, file_total_count))
                        prompt_info12.set("download progress: {:.1%} ".format(data_count / content_size))
                        prompt_info21.set("%s" % file_name)
                        message_progress_bar1["value"] = int(data_count * 1000 / content_size)
                        root.update()
            prompt_info13.set("the total progress: {:.1%} ".format(file_count / file_total_count))
...
# copy to the dest dir and clear the temp dir
...
if __name__ == '__main__':
    ...
    root.title('client auto update')  # title
    root.resizable(False, False)  

    frame = tkinter.Frame(root)
    frame1 = tkinter.Frame(frame)
    prompt_info11 = tkinter.StringVar(value="download file: (0/0)")
    ...
    prompt_info12 = tkinter.StringVar(value="download progress: ")
    ...
    prompt_info13 = tkinter.StringVar(value="total progress: ")
    ...
    root.destroy()
    if update_flag and (module_name == "regressiontest" or module_name == "all_module"):
        # print("restart")
        restart_client()
